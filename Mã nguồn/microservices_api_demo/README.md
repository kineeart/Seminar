...
From the project root (`microservices_api_demo`), run:

```bash
# 1) Build all service jars (required because Dockerfiles copy from target/)
...
If PowerShell says `docker` is not recognized, close and reopen terminal (or reboot) so PATH is refreshed after Docker Desktop installation.
